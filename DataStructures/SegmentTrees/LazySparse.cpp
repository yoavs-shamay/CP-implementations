...
struct SegmentTree
{
    ...
    struct Node
    {
        ...
        T value;
        T lazy;
        Node(T val, T lazyVal) : value(val), lazy(lazyVal), left(NULL), right(NULL) {}
        Node *getLeft(T def, T lazyDef)
        {
            if (!left) left = new Node(def, lazyDef);
            return left;
        }
        Node *getRight(T def, T lazyDef)
        {
    ...
    Node *root;
    SegmentTree(T defaultVal, operation<T> op, T lazyDefault, operation<T> lazyOp, ll len)
    {
    ...
    void _update(Node *cur, ll from, ll to, T updateBy, ll tl, ll tr)
    {
        _push(cur);
        if (tl >= from && tr <= to)
        {
    ...
    void update(ll from, ll to, T updateBy)
    {
